            ...
            @else
                @foreach ($data as $row)
                    <tr class="text-base bg-white">
                        ...
                        <x-ui.dialog id="form-dialog-{{ $row['id'] }}" class="top-[50%] left-[20%]">
                            <x-slot name="slotdialog">
                                ...
                                <form id="editForm-{{ $row['id'] }}" action="{{ route('subcategory.edit', $row['id']) }}" onsubmit="return submiteditForm({{ $row['id'] }}, event)" method="post">
                                    @csrf
                                    @method('PUT')
